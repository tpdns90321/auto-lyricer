...
class TestRunpodUVR:
    ...
    def test_supported_audio_extensions(self, mock_client_session, valid_config):
        ...
        expected_extensions = (AudioExtension.MP3, AudioExtension.OGG, AudioExtension.WAV)
    ...
    def test_initialization_with_missing_api_key(self, invalid_config_missing_key):
        with pytest.raises(ValueError, match="RUNPOD_API_KEY must be provided in the configuration"):
            RunpodUVR(invalid_config_missing_key)
    
    def test_initialization_with_missing_endpoint(self, invalid_config_missing_endpoint):
        with pytest.raises(ValueError, match="RUNPOD_UVR_ENDPOINT must be provided in the configuration"):
            RunpodUVR(invalid_config_missing_endpoint)
    
    @pytest.mark.asyncio
    @patch('app.stt.background_remover.runpod_uvr.ClientSession')
    async def test_remove_background_success_with_mp3(self, mock_client_session, valid_config, mp3_audio):
        uvr = RunpodUVR(valid_config)
        
        processed_audio_data = b"processed_vocals_data"
        mock_response_data = {
            "output": {
                "vocals": b64encode(processed_audio_data).decode("utf-8")
            }
        }
    ...
    @pytest.mark.asyncio
    @patch('app.stt.background_remover.runpod_uvr.ClientSession')
    async def test_remove_background_success_with_wav(self, mock_client_session, valid_config, wav_audio):
        uvr = RunpodUVR(valid_config)
    ...
    @pytest.mark.asyncio
    @patch('app.stt.background_remover.runpod_uvr.ClientSession')
    async def test_remove_background_api_error(self, mock_client_session, valid_config, mp3_audio):
        uvr = RunpodUVR(valid_config)
    ...
    @pytest.mark.asyncio
    @patch('app.stt.background_remover.runpod_uvr.ClientSession')
    async def test_remove_background_with_unsupported_format(self, mock_client_session, valid_config, aac_audio):
        uvr = RunpodUVR(valid_config)
    ...
    @pytest.mark.asyncio
    @patch('app.stt.background_remover.runpod_uvr.ClientSession')
    async def test_remove_background_with_empty_audio(self, mock_client_session, valid_config):
        uvr = RunpodUVR(valid_config)
