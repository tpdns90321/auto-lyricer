...
class ConcreteBackgroundRemover(BackgroundRemover):
    ...
    async def _remove_background(self, audio: Audio) -> Audio:
        return Audio(
            binary=b"processed_audio_data",
            extension=self._output_audio_extension
        )
...
class TestAudioWorker:
    ...
    def test_supported_audio_extensions_property(self, audio_worker: ConcreteAudioWorker):
        assert audio_worker.supported_audio_extensions == (AudioExtension.MP3, AudioExtension.WAV)
    
    @pytest.mark.asyncio
    async def test_convert_audio_with_supported_format(self, audio_worker: ConcreteAudioWorker, mp3_audio: Audio):
        result = await audio_worker._convert_audio(mp3_audio)
        assert result == mp3_audio
    
    @pytest.mark.asyncio
    async def test_convert_audio_with_unsupported_format(self, audio_worker: ConcreteAudioWorker, aac_audio: Audio):
        with patch('app.stt.abstract.convert_audio_extension') as mock_convert:
            expected_audio = Audio(binary=b"converted_data", extension=AudioExtension.MP3)
            mock_convert.return_value = expected_audio
            
            result = await audio_worker._convert_audio(aac_audio)
            
            mock_convert.assert_called_once_with(
                origin_audio=aac_audio,
                target_extension=AudioExtension.MP3
            )
...
class TestBackgroundRemover:
    ...
    def test_output_audio_extension_property(self, background_remover: ConcreteBackgroundRemover):
        assert background_remover.output_audio_extension == AudioExtension.OGG
    
    @pytest.mark.asyncio
    async def test_remove_background_with_supported_format(self, background_remover: ConcreteBackgroundRemover, mp3_audio: Audio):
    ...
    @pytest.mark.asyncio
    async def test_remove_background_with_unsupported_format(self, background_remover: ConcreteBackgroundRemover, aac_audio: Audio):
        with patch('app.stt.abstract.convert_audio_extension') as mock_convert:
            converted_audio = Audio(binary=b"converted_mp3_data", extension=AudioExtension.MP3)
...
class TestSpeechToText:
    ...
    def test_output_subtitle_extension_property(self, speech_to_text: ConcreteSpeechToText):
        assert speech_to_text.output_subtitle_extension == SubtitleExtension.SRT
    
    @pytest.mark.asyncio
    async def test_transcribe_with_supported_format(self, speech_to_text: ConcreteSpeechToText, wav_audio: Audio):
    ...
    @pytest.mark.asyncio
    async def test_transcribe_with_unsupported_format(self, speech_to_text: ConcreteSpeechToText, aac_audio: Audio):
        with patch('app.stt.abstract.convert_audio_extension') as mock_convert:
            converted_audio = Audio(binary=b"converted_wav_data", extension=AudioExtension.WAV)
            mock_convert.return_value = converted_audio
            ...
            mock_convert.assert_called_once_with(
                origin_audio=aac_audio,
                target_extension=AudioExtension.WAV
            )
            assert result.content == "Hello world transcription"
            assert result.extension == SubtitleExtension.SRT