...
@pytest_asyncio.fixture
async def subtitle_service(database) -> SubtitleService:
    repository = SubtitleRepository(database)
    service = SubtitleService(repository)
    return service


@pytest_asyncio.fixture
async def sample_subtitle(subtitle_service, video_id) -> Subtitle:
    dto = CreateSubtitle(
...
@pytest.mark.asyncio
async def test_create_subtitle(subtitle_service, video_id):
    dto = CreateSubtitle(
        language=Language.english,
    ...
    result = await subtitle_service.create_subtitle(dto)
...
@pytest.mark.asyncio
async def test_create_subtitle_with_invalid_video_id(subtitle_service):
    dto = CreateSubtitle(
        language=Language.english,
        content="Test subtitle content",
        file_format=SubtitleExtension.SRT,
...
@pytest.mark.asyncio
async def test_get_subtitle_by_instance_id(subtitle_service, sample_subtitle):
    result = await subtitle_service.get_subtitle_by_instance_id(
        sample_subtitle.instance_id
    )
    ...
    assert result.file_format == sample_subtitle.file_format
    assert result.video_instance_id == sample_subtitle.video_instance_id


@pytest.mark.asyncio
async def test_get_subtitle_by_instance_id_not_found(subtitle_service):
    result = await subtitle_service.get_subtitle_by_instance_id(999)

    assert result is None


@pytest.mark.asyncio
async def test_get_paginated_subtitles_by_video_instance_id(subtitle_service, video_id):
    ...
    result = await subtitle_service.get_paginated_subtitles(page=1, size=10, video_instance_id=video_id)
...
@pytest.mark.asyncio
async def test_get_paginated_subtitles_by_video_instance_id_not_found(subtitle_service):
    result = await subtitle_service.get_paginated_subtitles(page=1, size=10, video_instance_id=999)
...
@pytest.mark.asyncio
async def test_get_paginated_subtitles(subtitle_service, video_id):
    ...
    paginated = await subtitle_service.get_paginated_subtitles()
    ...
    # Test second page
    paginated = await subtitle_service.get_paginated_subtitles(page=2)
    ...
    paginated = await subtitle_service.get_paginated_subtitles(page=1, size=5)
    assert paginated.page == 1
    ...
    paginated = await subtitle_service.get_paginated_subtitles(page=4)
